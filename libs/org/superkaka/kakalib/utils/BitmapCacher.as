	...
	public class BitmapCacher
	{
		...
		static public function cacheBitmapMovie(source:DisplayObject, transparent:Boolean = true, fillColor:uint = 0x00000000, scale:Number = 1):Vector.<BitmapFrameInfo>
		{
			...
			else
			{
				...
				v_bitInfo = new Vector.<BitmapFrameInfo>(c, true);//长度为c的数组
				...
				while (i < c)
				{
					...
					var list_childMC:Array = searchChild(mc, MovieClip);//取得当前帧的所有movieClip子项
		...
		/**
		 *把二维位图数据转换为一维 Vector.<BitmapFrameInfo>数据 方便作为BitmapMovie播放
		 * @return 
		 * 
		 */		
		static public function changeGridToVector(gradBitDataInfo:GridBitmapDataInfo):Vector.<BitmapFrameInfo>
		{
			
			var bitmapframeArr:Vector.<BitmapFrameInfo>=new Vector.<BitmapFrameInfo>(gradBitDataInfo.row*gradBitDataInfo.column,true)
			var i:int=0
			var j:int=0	
			while(i<gradBitDataInfo.row){
				
				while(j<gradBitDataInfo.column){
					var bitmapframeInfo:BitmapFrameInfo=new BitmapFrameInfo()
					bitmapframeInfo.bitmapData=gradBitDataInfo.gridBitDataArray[i][j]
					bitmapframeArr[i*gradBitDataInfo.column+j]=bitmapframeInfo				
					j++
				}
